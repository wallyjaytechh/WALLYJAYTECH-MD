...
async function deleteCommand(sock, chatId, message, senderId) {
    try {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: '*🥹I need to be an admin to delete unnecessary messages🥹.*' }, { quoted: message });
            return;
        }
        ...
        // If no number provided and not replying/mentioning, show usage message
            ...
            await sock.sendMessage(chatId, { 
                text: '*🟩Please specify the number of messages to delete🟩.*\n\n*Usage:*\n\n• `.del 10` *- Delete last 10 messages from group*\n\n• `.del 7 @sender` *- Delete last 7 messages from @sender*\n\n• `.del 1` *(reply to message) - Delete last 1 messages from replied sender messages*' 
            }, { quoted: message });
            return;
        }
