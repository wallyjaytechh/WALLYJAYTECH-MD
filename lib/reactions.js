...
const defaultSettings = {
    ...
    includeMode: false,       // false = exclude mode, true = include mode
    numberList: []            // phone numbers to include/exclude
};
...
function isNumberInList(jid) {
    if (!settings.numberList || settings.numberList.length === 0) return null; // no list = no filter
    const phone = extractPhoneNumber(jid);
    if (!phone) return null;
    
    // Check if phone matches any number in list (partial match)
    const found = settings.numberList.some(num => phone.includes(num) || num.includes(phone));
    
    if (settings.includeMode) {
        return found; // true = should react, false = skip
    } else {
        return !found; // true = should react (not excluded), false = skip (excluded)
    }
}
...
async function handleAreactCommand(sock, chatId, message, isOwner) {
    try {
        ...
        if (!action) {
            ...
            const filterMode = settings.includeMode ? '✅ Include' : '🚫 Exclude';
            ...
            await sock.sendMessage(chatId, {
                text: `🎭 *AUTO-REACT SETTINGS*\n\n` +
                      ...
                      `━━━━━━━━━━━━━━━━━━━━\n` +
                      `💡 *Examples:*\n` +
                      `└ .areact include add 2348155763709\n` +
                      `└ .areact include add 23481,23490,23470\n` +
                      `└ .areact exclude add 2348155763709`,
                ...channelInfo
            });
            return;
        }
        ...
        // Include/Exclude commands
            ...
            if (sub === 'add') {
                ...
                if (numbers.length === 0) {
                    await sock.sendMessage(chatId, { text: `⚠️ *USAGE:* .areact include add <numbers>\n\n✨ *Example:* .areact include add 2348155763709\n✨ *Bulk:* .areact include add 23481,23490,23470`, ...channelInfo });
                    return;
                }
            ...
            if (sub === 'add') {
                const numbers = args.slice(2).join(' ').split(/[, ]+/).map(n => n.replace(/[^0-9]/g, '')).filter(n => n.length >= 7);
                if (numbers.length === 0) {
                    await sock.sendMessage(chatId, { text: `⚠️ *USAGE:* .areact exclude add <numbers>\n\n✨ *Bulk:* .areact exclude add 23481,23490,23470`, ...channelInfo });
                    return;
                }
            ...
            const page = parseInt(args[1]) || 1;
            const perPage = 20;
            const totalPages = Math.ceil(settings.numberList.length / perPage);
            const start = (page - 1) * perPage;
