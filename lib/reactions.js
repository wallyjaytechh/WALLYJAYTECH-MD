...
function isNumberInList(jid, message) {
    ...
    if (!phone && message?.key?.participant) {
        const pNum = extractPhoneNumber(message.key.participant);
        // If it's a LID (too long), try store
        if (pNum && pNum.length > 15) {
            try {
                const store = require('./lightweight_store');
                const contact = store.contacts[message.key.participant];
    ...
    // Fall back to jid
    if (!phone) {
        const jidNum = extractPhoneNumber(jid);
        if (jidNum && jidNum.length > 15) {
            try {
                const store = require('./lightweight_store');
...
if (settings.numberList.length > 0 && isGroup) {
    console.log(`🔍 AREACT GROUP | sender: ${senderId} | participant: ${message.key.participant} | remoteJidAlt: ${message.key.remoteJidAlt} | react: ${listResult}`);
}
